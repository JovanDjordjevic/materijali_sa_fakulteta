...
-- Kreira se pogled koji za svaki datum izdvaja ukupan iznos racuna koji su
-- napravljeni tog datuma
...
-- Za svaki datum izdvaja se ukupan iznos racuna koji su napravljeni do tog
-- datuma, ukljucujuci i njega
SELECT A.Datum, SUM(B.Iznos)
FROM IznosPoDatumu A, IznosPoDatumu B
WHERE A.Datum >= B.Datum
GROUP BY A.Datum;

-- Unos podataka za domaci