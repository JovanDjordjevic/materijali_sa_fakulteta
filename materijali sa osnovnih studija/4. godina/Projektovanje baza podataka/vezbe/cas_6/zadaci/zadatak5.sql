...
CREATE TABLE ROBA (
    ...
    SifD INT,
    PRIMARY KEY(SifR),
    CONSTRAINT fk_sifd FOREIGN KEY (SifD) REFERENCES DOBAVLJAC(SifD)
...
CREATE TABLE NABAVKA (
    ...
    SifR INT NOT NULL,
    CONSTRAINT fk_sifr FOREIGN KEY (SifR) REFERENCES ROBA(SifR)
    ON DELETE CASCADE ON UPDATE CASCADE,
    --
    UNIQUE (SifR, Datum)
);
...
FOR EACH ROW
    BEGIN
        ...
        ELSEIF (NEW.Cena >= 1.1 * (SELECT N.Cena FROM NABAVKA N 
                                   WHERE N.SifR = NEW.SifR AND
                                        N.Datum = (SELECT MAX(N2.Datum) FROM NABAVKA N2 
