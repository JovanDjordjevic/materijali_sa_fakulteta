...
CREATE TABLE student_marks (
    ...
    GRADE VARCHAR(20),
    CONSTRAINT fk_student_id FOREIGN KEY (STUDENT_ID) REFERENCES student_mast(STUDENT_ID)
    ON DELETE CASCADE ON UPDATE CASCADE
);


--triger koji prilikom izmene ocna studentu u tabeli student_marks racuna polja
-- total, per_marks i grad
DELIMITER $$

CREATE TRTIGGER bu_student_marks
...
-- ispisemo definicije da vidimo kako se napravilo
SHOW CREATE TABLE student_mast;
SHOW CREATE TABLE student_marks;
-- istu stvar imamo i za trigere 
SHOW CREATE TRIGGER bu_student_marks;


-- ovaj triger se ne trazi eksplicitno u zadatku, ali kao za vezbu, triger koji
-- ce da izracuna ime studenta prilikom unosa u tabelu stuudent_marks
DELIMITER $$

CREATE TRIGGER bi_student_marks
BEFORE INSERT ON student_marks
FOR EACH ROW
    SET NEW.name = (SELECT name FROM stundet_mast WHERE STUDENT_ID = NEW.STUDENT_ID);

DELIMITER ;

--unosimo neke podatke za proveru
INSERT INTO student_mast VALUES
    ('mi15100', 'Pera Peric', 1),
    ('mi15120', 'Laza Lazic', 2);

-- kada unosimo u ovu tabelu ne moramo bas sve atribute da navodimo zato sto smo
-- psotavilis ta im je default vrednost
...
-- radimo azuriranje kako bi testirali triger koji se trazi u zadatku
