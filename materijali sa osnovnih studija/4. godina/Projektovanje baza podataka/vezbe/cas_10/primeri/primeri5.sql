...
-- spoljasnja petlja je ovde Employees, a Departments je unutrasnja (?)
explain analyze
select e.name, d.mgr
...
explain analyze
select e.name, d.mgr
from Departments as d, Employees as e -- promenili redosled ovde
where e.did = d.id;

explain analyze
select e.name, d.mgr
from Departments as d, Employees as e  -- promenili redosled ovde
where d.id = e.did;                    -- promenili redosled ovde
...
Kojim god smo redom navodili, optimizator je uvek sam zakljucio kako mu
vise odgovara (?)
*/

-- ako pogledamo ispis iz show profiles na dnu primera, vidimo da se desava neki 
-- inner hash join. Kada ne postoji indeks, kada sespoljasnja petlja prodje
-- jednom napravi se za nju neka hash tabela u radnoj memoriji i onda se unutrasnja
-- petlja pretrazuje pomocu te ahsh tabele (iako smo rekli da u innodb nemamo 
-- hash indekse, ovo nije hash indeks pa je dozovoljeno (?) )


-- gledamo kako se ponasaju 2 razlicita neklasterovana indeksa
-- Kreiraju se indeksi nad atributima spajanja.
-- prvi indeks
create index Departments_id_index on Departments (id) using btree;

-- uzmemo neku bilo koju varijantu prolog upita
explain analyze
select e.name, d.mgr
from Employees as e, Departments as d
where e.did = d.id;
...
Ako indeks nije klasteroan on moze usporiti pretragu, dakle ako je moguce 
treba napraviti klasterovanje (?)
*/



-- gledamo kako se ponasaju klasterovani. Rekla je da ne mora primarni kljuc
-- nego je dovoljno samo unique (?)
...
-- drugi klasterovani indeks
-- ovde posto did nije unique moramo da dodamo i id (?)
