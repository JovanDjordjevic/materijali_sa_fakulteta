...
final class InetAddressVersion {
	...
	private static int getVersion(InetAddress addr) {
		...
		// extending InetAddress, such as InetSocketAddress!  (zbog ovoga mora else if a ne samo else!)
	...
	private static void printAddress(byte[] address) {
		...
		for (byte b : address) {
			// InetAddress bytes need to be shifted by 256 if they are negative (zbog unsigned)
