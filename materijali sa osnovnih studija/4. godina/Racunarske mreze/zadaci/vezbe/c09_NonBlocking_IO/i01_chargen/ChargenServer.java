...
final class ChargenServer {
	...
	public static void main(String[] args) {
		...
		//bice nam potreban po jedan bytebuffer za svakog klijenta
		//pseudokod kako radimo:
		//List<Client>
		//	try to accept
		//  foreach client
		//		write buffer, if fails remove client
		//		if not reamining
		...
		for (byte i = ' '; i <= '~'; i++) {
			rotation[i - ' '] = i;       // pozicija na koju ubacujemo nije samo [i], nego je [i - space] zato sto ' ' nije 0 u ASCII tabeli
			rotation[i + 95 - ' '] = i;
		}
		...
		//posto treba na neki nacin da pratimo koje klijente treba da usluzijemo, mozemo koristiti Selector klasu
		try (ServerSocketChannel serverChannel = ServerSocketChannel.open();
			...
			// Bind to port(posto nigde u ServerSocketChannel.open() nismo naveli port
			...
			while (true) {
				...
				while (it.hasNext()) {
					...
					try {
						if (key.isAcceptable()) { //u nasem slucaju, u ovoj grani key ce biti SocketChannel
							ServerSocketChannel server = (ServerSocketChannel) key.channel(); //ono sto vrati .channel() moramo da castujemo
							SocketChannel client = server.accept(); //accept ce nam vratiti klijenta
							...
							//register moze kao povratnu vrednost da vrati kljuc koji je dodao u svoju internu selekciju
							SelectionKey clientKey = client.register(selector, SelectionKey.OP_WRITE);

							ByteBuffer buffer = ByteBuffer.allocate(74);
							// Copy initial line from rotation into the buffer
							//necemo bas 74 jer hocemo i \r\n za novi red (da bi bilo portable, na windowsu je novi red \r\n)
							...
							//za svakog klijenta kacimo po bafer bafer. Necemo da kacimo na key zato sto je u ovom trenutku key nas server, a ne klijent
							clientKey.attach(buffer);
						//} else if(key.isReadable()){  //NOTE: nas server nema citanje od strane klijenta tako da nam ovaj else if ne treba, ali dodali bismo ga da imamo
						} else if (key.isWritable()) {
							SocketChannel client = (SocketChannel) key.channel();
							ByteBuffer buffer = (ByteBuffer)key.attachment(); //ovde smo dohvatili bafer za ovog klijenta koji smo gore prikacili metodom .attach()
																			  //posto 	attachment() vraca Object, moramo da kastujemo u konkretno sta zelimo

							// If buffer is empty, refill the buffer with the next line
							if (!buffer.hasRemaining()) {
								// Rewind back to the start of the buffer (ovo vraca position na pcoetak)
								buffer.rewind();
								// Get the first character - so we can determine next line (get vraca ono na sta pokazuue position,i pomeri position unaperd)
					...
					//ako se neki klijent discnectuje, imaemo IOException, a ako ne dodamo catch za to ovde, desice se da se taj
					//IOException obradjuje van nase petlje, tj da server prestane sa radom posle diskonekcije nkeog klijenta
