...
final class URLIntro {
	...
	public static void main(String[] args) {
		...
		//URI dodje kao bazna klasa od URL i URN
		//URI: scheme:[//authority]path[?query][#fragment]  ,npr file://1.txt je sasvim validan URI ali nam je beskoristan
		//posto je URI nadskup od URL, ako mi npr iz URL izostavimo neki deo, dobicemo URI, ali to namje beskorisno za rad
		...
		try {
			//URL klasa ima mnogo razlicitih konstruktora, ali mi cemo najcesce koristiti ovaj koji samo primi string od kog zalimo da pravimo URL
			//ovi konstruktori nas forsiraju d a obradimo MalformedURLException. NOTE: u konstruktoru se NE ostvaruje konekcija sa hostom,
			// tako da ako prosledimo nevalidan url (u smislu nepostojecu adresu, npr:  nevalidnaadresa.bg.ac.rs) necmeo dobiti nikakvu gresku.
			//Konstruktor radi parsiranje proseldjenog stringa na nekle njegove delove, a ako to parsiranje ne uspe (tj ako ne pratimo pravilan nacin zapisa URL-a
			//onda dobijamo MalformedURLException
			u = new URL("http://www.matf.bg.ac.rs:8080");

			//imamo metode koje za nas dohvataju parsirane delove url-a
			...
			System.out.println("The port part of " + u + " is " + u.getPort());				//-1 jer nije naveden
			System.out.println("The default port for " + u + " is " + u.getDefaultPort());  //ovo ce zavisiti od protokola, npr za http je 80
			System.out.println("Path part of " + u + " is " + u.getPath());					//~ivan_ristovic/index.html
			System.out.println("Authority part of " + u + " is " + u.getAuthority());       //www.matf.bg.ac.rs
			System.out.println();
			...
			URLConnection uc = u.openConnection();		//ovde se tek desava konekcija. ovo baca IOException

			//iz URLCOnnection objekta mozemo ugradjenim metodama uzimati razne stvari iz odgovora servera
