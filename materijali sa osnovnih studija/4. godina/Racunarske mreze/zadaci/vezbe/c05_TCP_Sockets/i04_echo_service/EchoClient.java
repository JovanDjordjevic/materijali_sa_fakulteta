...
final class EchoClient {
	...
	// check the `Allow parallel run` option (U gornjem desnom uglu)
	...
	public static void main(String[] args) {
		...
		try (Socket sock = new Socket(hostname, EchoServer.DEFAULT_PORT);
			 BufferedReader networkIn = new BufferedReader(new InputStreamReader(sock.getInputStream(), StandardCharsets.UTF_8));
			 BufferedWriter networkOut = new BufferedWriter(new OutputStreamWriter(sock.getOutputStream(), StandardCharsets.UTF_8));
			 BufferedReader userIn = new BufferedReader(new InputStreamReader(System.in))  //mogli smo i scanner ovde, nije mroao reader
		) {
