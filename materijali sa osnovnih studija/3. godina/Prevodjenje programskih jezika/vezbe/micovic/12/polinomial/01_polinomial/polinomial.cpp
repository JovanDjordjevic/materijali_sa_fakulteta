...
polinomial* polinomial::izvod() const
{
    std::vector<float> coefs(m_coef.size() - 1);

    for (unsigned i = 0; i < coefs.size(); i++) {
        coefs[i] = (i + 1) * m_coef[i+1];
    }

    return new polinomial(coefs);
}

polinomial* polinomial::integral(float c) const
{
    std::vector<float> coefs(m_coef.size() + 1);

    for (unsigned i = 1; i < coefs.size(); i++) {
        coefs[i] = 1.0f / i * m_coef[i-1];
    }
    coefs[0] = c;
...
float polinomial::value(float x) const
{
    float result = 0;

    for (auto coef = m_coef.crbegin(); coef != m_coef.crend(); coef++) {
        // coef je iterator koji se krece unazad kroz vektor
