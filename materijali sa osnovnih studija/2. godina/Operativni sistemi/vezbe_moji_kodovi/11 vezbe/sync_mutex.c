...
void* threadFunc(void *arg){
    ...
    check_error(feof(in),"feof failed");
    
    check_thread(pthread_mutex_lock(&globalLock),"lock failed");
    globalSum+=localSum;
...
int main(int argc,char **argv){
   ...
   //check_thread(pthread_mutex_init(&globalLock,NULL),"...");  
   //ovo ide ako ne inicijalizujemo staticki kao gore
